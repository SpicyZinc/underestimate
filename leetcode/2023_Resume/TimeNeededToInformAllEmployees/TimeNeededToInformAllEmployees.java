...
idea:
dfs
from manager to all its members
the max will be the time to notify all
...
class TimeNeededToInformAllEmployees {
    ...
    private int dfs(Map<Integer, List<Integer>> graph, int[] informTime, int current) {
        int max = 0;
        if (!graph.containsKey(current)) return max;
