...
class MinimumDeletionsForAtMostKDistinctCharacters {
    public int minDeletion(String s, int k) {
        ...
        // Step 2: Store frequencies of used characters only
        List<Integer> frequencies = new ArrayList<>();
        for (int f : freq) {
            if (f > 0) frequencies.add(f);
        }

        // Step 3: If distinct characters already <= k, no deletion needed
        if (frequencies.size() <= k) return 0;

        // Step 4: Sort frequencies (smallest first to delete cheaply)
        Collections.sort(frequencies);

        int deletions = 0;
        int removeCount = frequencies.size() - k; // how many distinct characters to remove

        for (int i = 0; i < removeCount; i++) {
            deletions += frequencies.get(i); // delete entire character type
        }
