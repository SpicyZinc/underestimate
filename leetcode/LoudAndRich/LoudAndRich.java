...
but it isn't clear if they have more money than person 0.
...
idea:
dfs()
need to go back and visit again
*/
...
class LoudAndRich {
	public int[] loudAndRich(int[][] richer, int[] quiet) {
		...
		for (int[] edge : richer) {
			...
			int richPerson = edge[1];
			graph[richPerson].add(richerPerson);
		}
		// initialization for result
		Arrays.fill(result, -1);

		for (int i = 0; i < n; i++) {
			dfs(i, quiet, graph, result);
		}

		return result;
	}

	public int dfs(int person, int[] quiet, ArrayList<Integer>[] graph, int[] result) {
		if (result[person] == -1) {
			// self is the most quiet person
			int mostQuiet = person;
			for (int child : graph[person]) {
				int candidate = dfs(child, quiet, graph, result);
				if (quiet[candidate] < quiet[mostQuiet]) {
					mostQuiet = candidate;
				}
			}
			result[person] = mostQuiet;
		}
