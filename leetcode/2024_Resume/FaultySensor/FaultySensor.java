...
class FaultySensor {
    public int badSensor(int[] sensor1, int[] sensor2) {
        boolean canOneToTwo = canShift(sensor1, sensor2); // can sensor1 be shifted to sensor2?
        boolean canTwoToOne = canShift(sensor2, sensor1); // can sensor2 be shifted to sensor1?
        // both true or both false, then we can't tell
        if (canOneToTwo == canTwoToOne) return -1;
        return canOneToTwo ? 1 : 2;
    }

    // return true if s can be shifted to t, means s is defective, t is right
    private boolean canShift(int[] s, int[] t) {
        int i = 0;
        int j = 0;
        ...
        // there should be only 1 mismatch, so i has to be at last position, s.length - 1
        return i == s.length - 1;
    }
}
