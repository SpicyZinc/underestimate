...
map
key is key
value is combined value (value + timestamp)
from back to iterate over hm.get(key);
note ts <= timestamp
*/
...
class TimeMap {
	...
	public void set(String key, String value, int timestamp) {
		String combinedValue = value + "-" + timestamp;
		hm.computeIfAbsent(key, x -> new ArrayList<>()).add(combinedValue);
	}
	...
	public String get(String key, int timestamp) {
		if (hm.containsKey(key)) {
			...
			for (int i = list.size() - 1; i >= 0; i--) {
				String[] matches = list.get(i).split("-");
				String value = matches[0];
				int ts = Integer.parseInt(matches[1]);
