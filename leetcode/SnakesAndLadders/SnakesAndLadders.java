...
idea:
https://zhuanlan.zhihu.com/p/45339906
BSF to find minimum steps
*/
...
class SnakesAndLadders {
		...
		while (!queue.isEmpty()) {
			...
			for (int i = size - 1; i >= 0; i--) {
				...
				for (int j = 1; j <= 6 && num + j <= n * n; j++) {
					...
					// direct to that square, if not -1
					if (!visited[next]) {
						queue.offer(next);
					}
				}
	...
	public int getValue(int[][] board, int idx) {
		int n = board.length;

		int row = (idx - 1) / n;

		int x = n - 1 - row;
		int y = row % 2 == 0 ? idx - 1 - row * n : n + row * n - idx;
