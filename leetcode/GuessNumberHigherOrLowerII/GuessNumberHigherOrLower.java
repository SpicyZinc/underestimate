...
After our guess, the problem is divided into two subproblems. Notice we do not need to pay the money for both subproblems.
...
public class GuessNumberHigherOrLower {
    ...
    public int solve(int[][] dp, int L, int R) {
    	...
    	int dp[L][R] = Integer.MAX_VALUE;
    	for (int i = L; i <= R; i++) {
    		int temp = i + Math.max(solve(dp, L, i-1), solve(dp, i+1, R));
    		if (temp < dp[L][R]) {
    			dp[L][R] = temp;
    		}
    	}
