...
one certain bit of this number adds up 3 times, must be multiples of 3, modulus 3 to get it back to zero
...
public class SingleNumber {
    ...
    // Tue May  9 21:42:41 2023
    public int singleNumber(int[] nums) {
        int theSingleNumber = 0;
        // for each bit
        // 000100
        // 001000
        // 000010
        ...
        for (int i = 0; i < 32; i++) {
            ...
            for (int num : nums) {
                // 把所有数字这个位置i的bit 加起来
                // 凡是出现三次的 都 module 为0了
                // 不为0的就是要找的出现那唯一一次的number
                // 但是要归位 所以要 移回去
                if ((num >> i & 1) == 1) {
                    bitSum += 1;
                    bitSum %= 3;
            ...
            if (bitSum != 0) {
                theSingleNumber |= bitSum << i;
            }
        }
        return theSingleNumber;
    }
