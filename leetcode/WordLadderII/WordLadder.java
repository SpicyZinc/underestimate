...
class Entry {
	...
	public Entry(String word, int level, Entry prev) {
		this.word = word;
		this.level = level;
		this.prev = prev;
	}
...
class WordLadder {
	...
	public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
		...
		List<Entry> paths = new ArrayList<Entry>();
		...
		while (!queue.isEmpty()) {
			...
			// prune, because we want the shortest path
			...
			for (int i = 0; i < chars.length; i++) {
				...
				for (char c = 'a'; c <= 'z'; c++) {
					...
					// found a path
					if (hs.contains(word) && word.equals(endWord)) {
						minLevel = curr.level;
		...
		for (Entry path : paths) {
			List<String> ladder = new ArrayList<>();
			while (path != null) {
				ladder.add(0, path.word);
				path = path.prev;
			}
			result.add(ladder);
		}
