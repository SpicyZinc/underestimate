...
public class LetterCombinationsOfAPhoneNumber {
    ...
    public List<String> letterCombinations(String digits) {
        ...
        String[] s = hm.get(digits.charAt(0)).split("");
        for (int i = 1; i < digits.length(); i++) {
            String[] t = hm.get(digits.charAt(i)).split("");
            s = compose(s, t);
        }
    ...
    // Wed May  1 03:23:06 2024
    public String[] compose(String[] s, String[] t) {
        String[] result = new String[s.length * t.length];
        int idx = 0;
        for (int i = 0; i < s.length; i++) {
            for (int j = 0; j < t.length; j++) {
                String product = s[i] + t[j];
                result[idx++] = product;
            }
        }
        return result;
