...
idea:
two maps to save id to importance, id to subordinates
while (queue.isEmpty()) 
*/
...
class EmployeeImportance {
    public int getImportance(List<Employee> employees, int id) {
		Map<Integer, List<Integer>> directSubordinates = new HashMap<Integer, List<Integer>>();
		...
		for (Employee employee : employees) {
			...
			directSubordinates.put(employee.id, employee.subordinates);
			if (employee.id == id) {
				master = employee;
			}
		}

		Queue<Integer> queue = new LinkedList<>(master.subordinates);
		int result = master.importance;
		while (!queue.isEmpty()) {
			int eid = queue.poll();
			result += importance.get(eid);
			for (int sub : directSubordinates.get(eid)) {
				queue.add(sub);
			}
		}
