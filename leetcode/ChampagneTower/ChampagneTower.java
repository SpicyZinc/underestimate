...
Output: 0.5
Explanation: We poured 2 cups of champagne to the top glass of the tower (which is indexed as (0, 0)). There is one cup of excess liquid.
The glass indexed as (1, 0) and the glass indexed as (1, 1) will share the excess liquid equally, and each will get half cup of champagne.
...
use pascal triangle,
i (row_index)
i + row_index + 1, i + row_index + 2
*/

class ChampagneTower {
	public double champagneTower(int poured, int query_row, int query_glass) {
		int total = (0 + (query_row + 1)) * (query_row + 1 + 1) / 2 + (query_row + 1 + 1); // purposely added one more row
		double[] triangle = new double[total];
		triangle[0] = poured * 1.0;
		// index in the triangle[]
		int index = 0;
		for (int i = 0; i <= query_row; i++) {
			for (int j = 0; j <= i; j++) {
				if (triangle[index] >= 1) {
					triangle[index + i + 1] += (double) ((triangle[index] - 1.0) / 2.0);
					triangle[index + i + 2] += (double) ((triangle[index] - 1.0) / 2.0);
					triangle[index] = 1.0;
				}
				index += 1;
			}
		...
		// after population
		return triangle[query_row * (query_row + 1) / 2 + query_glass];
	}
}